class ProductManager {
    ...
    createProductCard(product) {
        ...
        card.innerHTML = `
            ...
            <div class="product-price">R$ ${parseFloat(product.preco || 0).toFixed(2)}</div>
            <div class="product-quantity ${quantityClass}">
                Estoque: ${product.quantidade} unidades
            </div>
            ${product.data_validade ? `
                <div class="product-date">
                    <i class="fas fa-calendar"></i> Validade: ${new Date(product.data_validade).toLocaleDateString('pt-BR')}
                </div>
            ` : ''}
    ...
    updateProductCount() {
        ...
        countElement.textContent = `${this.products.length} produto${this.products.length !== 1 ? 's' : ''}`;
    }
    ...
    async saveProduct() {
        ...
        try {
            ...
            this.showSuccess(this.isEditing ? 'Produto atualizado com sucesso!' : 'Produto cadastrado com sucesso!');
            this.closeModal();
            await this.loadProducts();

        } catch (error) {
            console.error('Erro ao salvar produto:', error);
            this.showError(error.message || 'Erro ao salvar produto. Tente novamente.');
        }
    }
    ...
    validateForm() {
        ...
        if (!name) {
            ...
            this.showFieldError('productName', 'Nome deve ter pelo menos 2 caracteres');
            isValid = false;
        }

        // Validar preço de compra
        if (!priceBuy || priceBuy <= 0) {
            this.showFieldError('productPriceBuy', 'Preço de compra deve ser maior que zero');
            isValid = false;
        }

        // Validar preço de venda
        if (!priceSell || priceSell <= 0) {
            this.showFieldError('productPriceSell', 'Preço de venda deve ser maior que zero');
            isValid = false;
        }

        // Validar se preço de compra é menor que preço de venda
        if (priceBuy && priceSell && parseFloat(priceBuy) >= parseFloat(priceSell)) {
            this.showFieldError('productPriceBuy', 'Preço de compra deve ser menor que preço de venda');
            this.showFieldError('productPriceSell', 'Preço de venda deve ser maior que preço de compra');
            isValid = false;
        }

        // Validar quantidade
        if (!quantity || quantity < 0) {
            this.showFieldError('productQuantity', 'Quantidade deve ser maior ou igual a zero');
            isValid = false;
        }
        ...
        if (validity) {
            ...
            if (validityDate < today) {
                this.showFieldError('productValidity', 'Data de validade não pode ser anterior a hoje');
                isValid = false;
            }
        }
    ...
    async deleteProductById(id) {
        try {
            ...
            this.showError(error.message || 'Erro ao excluir produto. Tente novamente.');
        }
    }
    ...
    openModal(product = null) {
        ...
        if (this.isEditing) {
            ...
            document.getElementById('productPriceBuy').value = product.precocompra || '';
            ...
            document.getElementById('productValidity').value = product.data_validade ? product.data_validade.split('T')[0] : '';
        } else {
            form.reset();
        }
                ...
                if (product) {
                    ...
                    details.innerHTML = `
                        <div style="display: grid; gap: 1.5rem;">
                            <div>
                                <h3 style="color: #1e40af; margin-bottom: 0.5rem;">${product.nome}</h3>
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                                <div>
                                    ...
                                    <div style="color: ${product.quantidade < 10 ? '#dc2626' : '#059669'}; font-weight: 600;">
        ...
        window.deleteProduct = function(id) {
            ...
            if (productManager) {
                ...
                if (product) {
                    ...
                    const productName = confirmModal.querySelector('#productToDeleteName');
                    if (productName) {
